...
impl EnvironmentManager {
    ...
    pub fn get_path_entries(&self) -> Vec<PathBuf> {
        vec![
            ...
            self.store_path.join("lib"),
        ]
    }
    
    /// Generate shell script to set up environment
    pub fn generate_shell_setup(&self, shell: &str) -> Result<String, Box<dyn std::error::Error>> {
...
"#,
    ...
    pub async fn install_shell_integration(&self, shell: &str) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if !existing_content.contains("# Pyro package manager environment setup") {
            ...
            println!("   Please restart your shell or run: source {}", config_file.display());
        } else {
    ...
    pub async fn remove_shell_integration_async(&self, shell: &str) -> Result<(), Box<dyn std::error::Error>> {
        let home_dir = dirs::home_dir()
            .ok_or("Could not determine home directory")?;
        ...
        let config_file = match shell {
            ...
            "zsh" => home_dir.join(".zshrc"),
            "fish" => home_dir.join(".config/fish/config.fish"),
            "powershell" | "pwsh" => home_dir.join("Documents/PowerShell/Microsoft.PowerShell_profile.ps1"),
            _ => return Err(format!("Unsupported shell: {}", shell).into())
        };
        ...
        for line in lines {
            ...
            if skip_pyro_section {
                ...
                if line.trim().is_empty() || (line.starts_with('#') && !line.contains("Pyro")) {
    ...
    pub fn get_package_environment(&self, package_hash: &str) -> HashMap<String, String> {
        ...
        env.insert("PYRO_PACKAGE_PATH".to_string(), package_path.to_string_lossy().to_string());
        env.insert("PYRO_PACKAGE_BIN".to_string(), bin_path.to_string_lossy().to_string());
        env.insert("PYRO_PACKAGE_LIB".to_string(), lib_path.to_string_lossy().to_string());
    ...
    pub fn get_environment_variables(&self) -> HashMap<String, String> {
        ...
        env.insert("PYRO_STORE_PATH".to_string(), self.store_path.to_string_lossy().to_string());
        env.insert("PYRO_BIN_PATH".to_string(), bin_path.to_string_lossy().to_string());
        env.insert("PYRO_LIB_PATH".to_string(), lib_path.to_string_lossy().to_string());
        
        // Add current PATH modification
        if let Ok(current_path) = std::env::var("PATH") {
            let path_separator = if cfg!(windows) { ";" } else { ":" };
            let new_path = format!("{}{}{}", bin_path.to_string_lossy(), path_separator, current_path);
    ...
    pub fn setup_shell_integration(&self, shell: &str) -> Result<(), Box<dyn std::error::Error>> {
        let rt = tokio::runtime::Runtime::new()?;
        rt.block_on(self.install_shell_integration(shell))
    }
    
    /// Remove shell integration
    pub fn remove_shell_integration(&self, shell: &str) -> Result<(), Box<dyn std::error::Error>> {
        let rt = tokio::runtime::Runtime::new()?;
        rt.block_on(self.remove_shell_integration_async(shell))
    }
    
    /// Generate shell script
    pub fn generate_shell_script(&self, shell: &str) -> Result<String, Box<dyn std::error::Error>> {
