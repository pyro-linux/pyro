...
use crate::config::{PackageSource, BuildConfig};
...
impl SystemBuilder {
    ...
    pub async fn install_dependency(&self, package: &str) -> Result<(), Box<dyn std::error::Error>> {
    ...
    /// Build a package
    pub async fn build_package(&self, spec: &SystemPackageSpec) -> Result<PathBuf, Box<dyn std::error::Error>> {
        self.build_system_package(spec).await
    }
    
    /// Build a system package
    pub async fn build_system_package(&self, spec: &SystemPackageSpec) -> Result<PathBuf, Box<dyn std::error::Error>> {
        println!("Building system package: {} v{}", spec.name, spec.version);
        
        // Create build directory
        let build_dir = self.config.cache_dir.join("system-builds").join(&spec.name);
        ...
        let source_dir = self.prepare_source_with_source(source, &build_dir).await?;
        
        // Set up build environment
        let env = self.setup_build_environment(spec)?;
        
        // Execute build based on build type
        let build_type = spec.build_type.as_ref().unwrap_or(&SystemBuildType::Autotools);
        let output_dir = match build_type {
            SystemBuildType::Autotools => self.build_autotools(spec, &source_dir, &env).await?,
            SystemBuildType::CMake => self.build_cmake(spec, &source_dir, &env).await?,
            SystemBuildType::Meson => self.build_meson(spec, &source_dir, &env).await?,
            SystemBuildType::Custom => self.build_custom(spec, &source_dir, &env).await?,
            SystemBuildType::Cargo => self.build_cargo(spec, &source_dir, &env).await?,
            SystemBuildType::LLVM => self.build_llvm(spec, &source_dir, &env).await?,
            SystemBuildType::Kernel => self.build_kernel(spec, &source_dir, &env).await?,
        };
    ...
    async fn prepare_source_with_source(&self, source: &PackageSource, build_dir: &PathBuf) -> Result<PathBuf, Box<dyn std::error::Error>> {
        ...
        match source {
            ...
            _ => return Err("Unsupported source type for system packages".into()),
        }
    ...
    /// Set up build environment
    fn setup_build_environment(&self, spec: &SystemPackageSpec) -> Result<HashMap<String, String>, Box<dyn std::error::Error>> {
        ...
        // Set up sysroot
        env.insert("SYSROOT".to_string(), self.sysroot.to_string_lossy().to_string());
        env.insert("PREFIX".to_string(), self.sysroot.to_string_lossy().to_string());
        
        // Set up PKG_CONFIG_PATH
        let pkg_config_path = self.sysroot.join("lib").join("pkgconfig");
        env.insert("PKG_CONFIG_PATH".to_string(), pkg_config_path.to_string_lossy().to_string());
        
        Ok(env)
    }
    
    /// Build using GNU Autotools
    async fn build_autotools(&self, spec: &SystemPackageSpec, source_dir: &PathBuf, env: &HashMap<String, String>) -> Result<PathBuf, Box<dyn std::error::Error>> {
        let install_prefix = spec.install_prefix.as_ref().unwrap_or(&self.sysroot);
        ...
        if !configure_output.status.success() {
            return Err(format!("Configure failed: {}", String::from_utf8_lossy(&configure_output.stderr)).into());
        }
        ...
        if !make_output.status.success() {
            return Err(format!("Make failed: {}", String::from_utf8_lossy(&make_output.stderr)).into());
        }
        ...
        if !install_output.status.success() {
            return Err(format!("Make install failed: {}", String::from_utf8_lossy(&install_output.stderr)).into());
        }
    ...
    /// Build using CMake
    async fn build_cmake(&self, spec: &SystemPackageSpec, source_dir: &PathBuf, env: &HashMap<String, String>) -> Result<PathBuf, Box<dyn std::error::Error>> {
        let install_prefix = spec.install_prefix.as_ref().unwrap_or(&self.sysroot);
        ...
        cmake_cmd.arg("..");
        cmake_cmd.arg(format!("-DCMAKE_INSTALL_PREFIX={}", install_prefix.display()));
        ...
        if !cmake_output.status.success() {
            return Err(format!("CMake configure failed: {}", String::from_utf8_lossy(&cmake_output.stderr)).into());
        }
        ...
        if !build_output.status.success() {
            return Err(format!("CMake build failed: {}", String::from_utf8_lossy(&build_output.stderr)).into());
        }
        ...
        let install_output = install_cmd.output()?;
        if !install_output.status.success() {
            return Err(format!("CMake install failed: {}", String::from_utf8_lossy(&install_output.stderr)).into());
        }
    ...
    /// Build using Meson
    async fn build_meson(&self, _spec: &SystemPackageSpec, _source_dir: &PathBuf, _env: &HashMap<String, String>) -> Result<PathBuf, Box<dyn std::error::Error>> {
        // Meson build implementation
        Err("Meson builds not yet implemented".into())
    }
    
    /// Build using custom script
    async fn build_custom(&self, spec: &SystemPackageSpec, source_dir: &PathBuf, env: &HashMap<String, String>) -> Result<PathBuf, Box<dyn std::error::Error>> {
        if let Some(script) = &spec.build_script {
            ...
            if !output.status.success() {
                return Err(format!("Custom build failed: {}", String::from_utf8_lossy(&output.stderr)).into());
            }
        }
        
        Ok(spec.install_prefix.as_ref().unwrap_or(&self.sysroot).clone())
    }
    
    /// Build Rust/Cargo project
    async fn build_cargo(&self, _spec: &SystemPackageSpec, _source_dir: &PathBuf, _env: &HashMap<String, String>) -> Result<PathBuf, Box<dyn std::error::Error>> {
        // Cargo build implementation
        Err("Cargo builds not yet implemented".into())
    }
    
    /// Build LLVM
    async fn build_llvm(&self, _spec: &SystemPackageSpec, _source_dir: &PathBuf, _env: &HashMap<String, String>) -> Result<PathBuf, Box<dyn std::error::Error>> {
        // LLVM build implementation
        Err("LLVM builds not yet implemented".into())
    }
    
    /// Build Linux kernel
    async fn build_kernel(&self, _spec: &SystemPackageSpec, _source_dir: &PathBuf, _env: &HashMap<String, String>) -> Result<PathBuf, Box<dyn std::error::Error>> {
...
impl SystemPackages {
    ...
    pub fn glibc(version: &str) -> SystemPackageSpec {
        SystemPackageSpec {
            ...
            source: Some(PackageSource::Url {
                url: format!("https://ftp.gnu.org/gnu/glibc/glibc-{}.tar.gz", version),
                hash: String::new(),
            }),
    ...
    pub fn llvm(version: &str) -> SystemPackageSpec {
        SystemPackageSpec {
            ...
            arch: "x86_64".to_string(),
            source: Some(PackageSource::Url {
                url: format!("https://github.com/llvm/llvm-project/releases/download/llvmorg-{}/llvm-{}.src.tar.xz", version, version),
                hash: String::new(),
            }),
    ...
    pub fn rust(version: &str) -> SystemPackageSpec {
        SystemPackageSpec {
            ...
            arch: "x86_64".to_string(),
            source: Some(PackageSource::Url {
                url: format!("https://forge.rust-lang.org/infra/channel-layout.html#source-code"),
                hash: String::new(),
            }),
            ...
            runtime_inputs: vec![],
            environment: HashMap::new(),
            build_script: Some("./configure --prefix=$PREFIX && make && make install".to_string()),
            configure_args: Some(vec![]),
            make_args: Some(vec![]),
            install_prefix: None,
            cross_compile_target: None,
