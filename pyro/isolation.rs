...
impl EnvironmentBuilder {
    ...
    pub async fn create_environment(&self, env_spec: &IsolatedEnvironment) -> Result<PathBuf, Box<dyn std::error::Error>> {
        let output_dir = &self.store_path.join("environments");
        std::fs::create_dir_all(output_dir)?;
        self.create_environment_with_output(env_spec, output_dir).await
    }
    
    /// Create an isolated environment directory with custom output
    pub async fn create_environment_with_output(&self, env_spec: &IsolatedEnvironment, output_dir: &PathBuf) -> Result<PathBuf, Box<dyn std::error::Error>> {
        ...
        self.install_packages_to_env(&env_spec.packages, &env_dir).await?;
        ...
        println!("✅ Environment '{}' created at: {}", env_spec.name, env_dir.display());
        Ok(env_dir)
    }
    
    /// Create standard Unix directory structure
    fn create_unix_structure(&self, env_dir: &PathBuf) -> Result<(), Box<dyn std::error::Error>> {
        let dirs = [
            "bin", "sbin", "lib", "lib64", "usr/bin", "usr/sbin", "usr/lib", "usr/lib64",
            "usr/include", "usr/share", "etc", "var", "tmp", "home", "root", "dev", "proc", "sys"
        ];
    ...
    async fn install_packages_to_env(&self, packages: &[String], env_dir: &PathBuf) -> Result<(), Box<dyn std::error::Error>> {
    ...
    fn find_package_in_store(&self, package_name: &str) -> Result<PathBuf, Box<dyn std::error::Error>> {
        ...
        for entry in store_entries {
            ...
            if path.is_dir() {
                ...
                if metadata_path.exists() {
                    ...
                    if metadata_content.contains(&format!("\"name\":\"{}\"", package_name)) {
                        return Ok(path);
                    }
                }
            }
    ...
    /// Copy package contents to environment
    fn copy_package_to_env(&self, package_path: &PathBuf, env_dir: &PathBuf) -> Result<(), Box<dyn std::error::Error>> {
        // Copy binaries
    ...
    fn copy_directory_contents(&self, src: &PathBuf, dst: &PathBuf) -> Result<(), Box<dyn std::error::Error>> {
    ...
    /// Set up environment configuration
    fn setup_environment_config(&self, env_spec: &IsolatedEnvironment, env_dir: &PathBuf) -> Result<(), Box<dyn std::error::Error>> {
        // Create /etc/environment
        let env_file = env_dir.join("etc/environment");
        let mut env_content = String::new();
        
        // Add standard PATH
        env_content.push_str("PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin\n");
    ...
    /// Generate container/VM configuration files
    fn generate_container_configs(&self, env_spec: &IsolatedEnvironment, env_dir: &PathBuf) -> Result<(), Box<dyn std::error::Error>> {
        // Generate Dockerfile
    ...
    /// Generate Dockerfile
    fn generate_dockerfile(&self, env_spec: &IsolatedEnvironment, env_dir: &PathBuf) -> Result<(), Box<dyn std::error::Error>> {
        ...
        // Set PATH
        dockerfile_content.push_str("ENV PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin\n\n");
    ...
    fn generate_docker_compose(&self, env_spec: &IsolatedEnvironment, env_dir: &PathBuf) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if !env_spec.mount_points.is_empty() {
            ...
            for mount in &env_spec.mount_points {
                ...
                compose_content.push_str(&format!("      - \"{}:{}{}\"\n", 
                    mount.host_path.display(), mount.container_path.display(), ro));
            }
        }
        
        // Resource limits
        if env_spec.resource_limits.memory_mb.is_some() || env_spec.resource_limits.cpu_cores.is_some() {
    ...
    /// Generate VM startup script
    fn generate_vm_script(&self, env_spec: &IsolatedEnvironment, env_dir: &PathBuf) -> Result<(), Box<dyn std::error::Error>> {
        let script_path = env_dir.join("start-vm.sh");
        let mut script_content = String::new();
        
        script_content.push_str("#!/bin/bash\n\n");
        script_content.push_str(&format!("# VM startup script for {}\n\n", env_spec.name));
        ...
        script_content.push_str(&format!("  -append \"root=/dev/ram0 init=/sbin/init\" \\\n"));
    ...
    pub async fn create_minimal_linux(&self, output_dir: &PathBuf) -> Result<PathBuf, Box<dyn std::error::Error>> {
    ...
    pub async fn enter_environment(&self, name: &str) -> Result<(), Box<dyn std::error::Error>> {
        ...
        println!("  docker run -it --rm -v {}:/workspace {}", env_dir.display(), name);
    ...
    pub async fn remove_environment(&self, name: &str) -> Result<(), Box<dyn std::error::Error>> {
        let env_dir = self.store_path.join("environments").join(name);
    ...
    pub async fn list_environments(&self) -> Result<Vec<IsolatedEnvironment>, Box<dyn std::error::Error>> {
        ...
        for entry in std::fs::read_dir(&envs_dir)? {
            ...
            if path.is_dir() {
                ...
                if metadata_path.exists() {
                    let metadata_content = std::fs::read_to_string(&metadata_path)?;
                    if let Ok(env) = serde_json::from_str::<IsolatedEnvironment>(&metadata_content) {
    ...
    pub async fn create_dev_environment(&self, output_dir: &PathBuf) -> Result<PathBuf, Box<dyn std::error::Error>> {
        let env_spec = IsolatedEnvironment {
