...
use crate::config::{PackageSource, PackageSpec};
...
impl RustcBuilder {
	...
	pub async fn build_with_rustc(
		&mut self,
		package_spec: &PackageSpec,
		build_log: &mut String,
	) -> Result<bool, BuildError> {
		build_log.push_str(
			"Starting rustc-based build with Pyro dependency graph\n",
		);
		...
		build_log.push_str(&format!(
			"Parsed Cargo.toml for package: {}\n",
			cargo_toml.package.name
		));

		// Convert Cargo dependencies to Pyro packages
		let packages =
			self.resolve_dependencies(&cargo_toml, build_log).await?;
	...
	async fn prepare_source(
		...
		match &package_spec.source {
			PackageSource::Crates { name, version } => {
				...
				self.download_crate(name, version, &source_dir, build_log)
					.await?;
			}
	...
	async fn resolve_dependencies(
		...
		if let Some(deps) = &cargo_toml.dependencies {
			for (name, dep) in deps {
				let package_spec =
					self.resolve_dependency_spec(name, dep).await?;
	...
	async fn resolve_dependency_spec(
		&mut self,
		name: &str,
		dep: &CargoDependency,
		...
		// Cache the result
		self.registry_cache
			.insert(name.to_string(), package_spec.clone());
		Ok(package_spec)
	}
	...
	async fn build_dependency(
		...
		if store_path.exists() {
			build_log.push_str(&format!(
				"Dependency {} already built, skipping\n",
				package.name
			));
			return Ok(());
		}
	...
	async fn build_main_package(
		...
		build_log.push_str(&format!(
			"Building main package: {} with rustc\n",
			cargo_toml.package.name
		));
		...
		let (source_file, is_binary) = if main_rs.exists() {
			...
			return Err(BuildError::BuildFailed(
				"No main.rs or lib.rs found".to_string(),
			));
		};
		...
		if is_binary {
			...
			cmd.arg("-o").arg(
				output_dir.join(format!("lib{}.rlib", cargo_toml.package.name)),
			);
		}
		...
		if output.status.success() {
			build_log.push_str(&format!(
				"Successfully built {} with rustc\n",
				cargo_toml.package.name
			));
			Ok(true)
		} else {
			build_log.push_str(&format!(
				"rustc build failed with exit code: {:?}\n",
				output.status.code()
			));
			Ok(false)
		}
